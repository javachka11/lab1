...
void sumtest_diftypes() {
	...
	char f = 0;
	double test_a[] = { 4.2, 1.5, 3.77, 4.4, 8.0 };
	double test_b[] = { -4.2, -1.5, -3.77, -4.4, -8.0, 2.1, 3.4, 3.2, 1.1, 0.22 };
	v1 = test_realVector(test_a, 5);
	v2 = test_complexVector(test_b, 5);
	v_sum = VectorSum(v1, v2);
	if ((char)(v_sum.arr == NULL))
...
void scprodcomplextest() {
	...
	if ((char)(a == NULL))
		return;
	if ((char)(VAbs(a->re - 8.581) < EPS) * (char)(VAbs(a->im - 52.7975) < EPS))
...
void scprodcomplextest_zero() {
	...
	if ((char)(a == NULL))
		return;
	if ((char)(VAbs(a->re - 0.0) < EPS) * (char)(VAbs(a->im - 0.0) < EPS))
